...
class TestCase(unittest.TestCase):
    ...
    def test_pray(self):
        ...
        p1 = Post(body="post from john", prayer_count=1,
                  timestamp=utcnow + timedelta(seconds=1))
        p2 = Post(body="post from susan", prayer_count=1,
                  timestamp=utcnow + timedelta(seconds=1))
        db.session.add(p1)
        db.session.add(p2)
        #print p1.prayer_count
        assert p1.prayer_count == 1
        assert p1.prayer_count == p2.prayer_count
        p1.prayer_count += 1
        assert p1.prayer_count == 2
        assert p1.prayer_count >= p2.prayer_count
